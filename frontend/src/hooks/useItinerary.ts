...
import type { TripActivity, TripApiModel, TripDayPlan, UseItineraryResult } from '@/types';

function updateDay(
  trip: TripApiModel,
  dayNumber: number,
  updater: (day: TripDayPlan) => TripDayPlan,
): TripApiModel {
  return {
    ...trip,
...
export function useItinerary(initialTrip: TripApiModel): UseItineraryResult {
  ...
  const commitTrip = (nextTrip: TripApiModel) => {
    setTrip(nextTrip);
